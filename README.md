...
**核心结果**：在归一化负载 1.4 的重载场景下，CA-MQR 平均端到端时延相比 Dijkstra 降低约 **62%**，三类业务 QoS 满足率均优于所有基线。

---

## 2. 仿真场景

- **网络节点**：66 颗卫星（Walker Delta 星座，6 轨道面 × 11 星/面，550 km，倾角 53°），采用 +Grid 星间链路拓扑（每星 4 个邻居：前/后/左/右）。
...
|----------------------|---------|------|-------------|--------|
| 平均端到端时延 (ms) | 278 | 163 | 147 | **106** |
| 链路利用率 (95 分位) | 0.94 | 0.84 | 0.79 | **0.69** |

- **时延**：CA-MQR 相比 Dijkstra 降低约 62%，且随负载增长增幅最小。
- **QoS**：三类业务满足率均为最优（A 类 0.64、B 类 1.00、C 类 1.00）。
- **消融**：去除拥塞感知与业务差异化后（Vanilla-DQN），重载时延由 106 ms 升至 147 ms，证明其为性能主要来源；竞争式网络结构主要提升训练稳定性。
- **势场增强**：在流量热点汇聚的严重拥塞场景下，下游拥塞势场进一步将 P95 尾部时延降低约 16%。
...
```
姚飞. 低轨卫星网络拥塞感知的多业务智能路由方法. 第 40 届南京地区研究生通信年会.
```
